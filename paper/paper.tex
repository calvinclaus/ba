...
The author of this paper used the following approach to find out if he encoded node failure correctly:
\begin{enumerate}
  \item Construct an assertion describing the system state that should be allowed to occur
  \item Negate the assertion
  \item Add the assertion to an appropriate location in the specification
  \item Check that assertion fails
  \item Remove assertion
\end{enumerate}
...
\end{lstlisting}
\end{mdframed}
after the \textit{simulate\_failure} action. The assertion checks that a node cannot fail and not deliver a message to J while successfully delivering a message to K. If IVy does not find a counterexample to this assertion, it means that the specification does not allow partial message delivery. The specification would therefore be erroneous. One has to finally remove the failing assertion to allow Ivy to proof induction again.

In light of this, it would be beneficial to have a built in mechanism to verify the possibility of states to occur. In this case, it would allow one to verify, that the modeled system allows for the possibility of a node to fail such that it only sends messages to a subset of processes, as part of the final proof.

Of course this approach does not constitute a proof of correct \textit{framework}-specification. It solely acts as a form of test for the user.
