...
Lines 2 to 5 resemble the snippet of section \textit{vii.1}. In line 2 we non-deterministically choose a number of nodes to fail. The \textit{assume} in line 3 then excludes all cases where a node that already failed was chosen to fail again. Line 4 adds the newly failed nodes to the set of failed nodes. Line 5 enforces $f < n$. Since this action is only called once per round we do not need an extra relation \textit{failing\_now} presented in \textit{vii.1}.
...
It has become apparent that correctly specifying an algorithm in IVy is not an easy feat. This sections summarizes and evaluates on the variety of challenges concerning correct specification that were mentioned in this paper.

\subsubsection{Equivalence of specification and algorithm}
...
\noindent \textbf{Structural difference of IVy and procedural languages}\\
...
\noindent \textbf{Lack of modularity}\\
...
\subsubsection{Miss-specification of system context}
...
\subsubsection{Necessity of deep understanding}
It has become apparent, that successful specification with IVy is not possible without deep knowledge about how the algorithm functions. IVy is not a ``translate and go'' solution.
Evidence include the alternative encoding for $f<n$ and for the decision condition $if\ rounds = f+1$.


\subsection{Verification challenges}

The following sections summarizes and evaluates on the variety of challenges concerning verification that were mentioned in this paper.

\subsubsection{Accidental invalidation of legal state configurations with \textit{assume}}
...
\subsubsection{Hard to discover inductive invariants}
Successfully discovering inductive invariants for \textit{Floodset} required a flawless understanding of the principles underlying the algorithm. Evidence of this includes the complex invariant that requires knowledge about \textit{lemma 1}. Users must be prepared to gain strong foundational understanding of the reasons why an algorithm works before attempting to find inductive invariants for it.


\subsubsection{Manual effort and practicability}
...
\subsubsection{Algorithm failure in theory and practice}
...
It highlighted the systems strengths to be user friendliness, interactivity and a powerful logic based language that allows for modeling of complex system state transitions in simple one-liners.

It concludes that IVy's user friendliness in presenting CTIs - so long as the user is vary of the presented limitations - enables finding inductive invariants for proving logical soundness of an algorithm relatively easily.
...
\begin{thebibliography}{99}
    ...
    \bibitem{wrongProof} P. Lincoln, J. Rushby. ``A formally verified algorithm for interactive consistency under a hybrid fault model.'', 1993
